...
- 刷怪系统：13 种地下城风怪物（哥布林、史莱姆、魔狼、巨魔、机械兽、精英龙人等），危险区自动刷新、仇恨追击、掉落材料/装备。
- 恋爱家庭：5 位美女 NPC、好感度、送礼约会告白、生孩子（最多 3 个，提供属性）。
- 部落系统：创建/加入/捐献/公告/酋长权限，部落等级提供全员属性加成。
...
- `POST /update` 更新坐标
- `POST /say` 发言
- `POST /action` 执行钓鱼、打工、交易、战斗等互动（`attack` 可带 `monsterId` 打怪，或不带则自动锁最近怪物）
- `POST /leave` 离开
- `GET /state` 返回字段含 `monsters`：当前存活怪物列表
